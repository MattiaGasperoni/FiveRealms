package view;

import java.util.List;

import model.characters.Character;

public class CharacterMovementMenu {
	
	
	
	public static void movementFace(Character attacker, List<Character> alliesList, List<Character> enemiesList) {
		// TODO Auto-generated method stub
		
	}
	
	/* Scegleire una struttura dati contenente tutti i blocchi dove il
	personaggio attacker si può spostare, se ce gia un personaggio in uno dei bocchi dove si può
	spostare , devo rimuovere il blocco 
	
	E metodo showGrid che evidenzia i blocchi in cui puoi spostarti
	*/ 
	

}
